...
1. **Create new assembly `Krypton.Toolkit.Themes`**
   ...
   - The `.csproj` **must** multi-target the same set of conditional TFMs as `Source/Krypton Components/Krypton.Toolkit/Krypton.Toolkit 2022.csproj` (currently `net472;net48;net6.0-windows;net7.0-windows;net8.0-windows;net10.0-windows`). Replicate `TargetFrameworks`, the `UseWindowsForms` property, `GenerateAssemblyInfo`, nullable/context settings and the common `Directory.Build.props` imports so the Themes DLL compiles identically across all supported platforms.
...
7. **Theme-name support**

   - **Persist**: `Export` adds a `ThemeName="<name>`" attribute on the `<KryptonPalette>` root element (or a `<ThemeName>` child when attribute cannot be used).
   - **Load/Upgrade**: `Import`, `ImportWithUpgrade` and `ImportFromXmlDocument` read the value and populate `PaletteName`.  If absent, default to file-name or embedded resource name.
   - **Validation**: During import, throw `ArgumentException` when the XML declares a version higher than `CURRENT_SUPPORTED_PALETTE_VERSION` _and_ a `ThemeName` is missing.
   - **Embedded themes**: `ThemeStore.LoadTheme` sets the returned palette’s `PaletteName` to the embedded file’s logical name; this value propagates to designer surfaces.
   - **Public API**: expose `string ThemeName { get; }` (read-only) on `PaletteBase` via a virtual forwarding property that returns `PaletteName` for backward compatibility.
   - **Unit tests**: `ThemeRoundTripTests.RoundTripKeepsThemeName()` writes a temp XML, imports it and asserts equality.
...
5. **Thread-safety sweep**
   - Audit static fields (`_suspendCount`, `_typeTests`, `_stringTests`, any ThemeStore caches).
   - Guard write operations with `Interlocked` or `lock` where contention is possible.
   - Replace non-atomic increment/decrement of `_suspendCount` with `Interlocked.Increment/Decrement` and document that UI calls are single-threaded.
   - Add `ThemeStore.ClearCache()` primarily for unit tests; ensure method is thread-safe.
   - No public behavioural changes; purely defensive.
