...
* Resolved [#3902](https://github.com/Krypton-Suite/Standard-Toolkit/issues/3902), Scrollbars leaves white gaps in between them and the borders
   ...
   * When both scrollbars are visible, the horizontal bar spans the full width to fill the bottom-right corner (vertical bar stops above it); set `KryptonScrollbarManager.CornerStyle` to `ScrollbarCornerStyle.ThemedCorner` for a themed corner filler with both bars shortened instead
