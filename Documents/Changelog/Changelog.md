...
* Implemented [#2117](https://github.com/Krypton-Suite/Standard-Toolkit/issues/2117), To save & load palettes to/from 'palette' binary
  ...
  * Older XML palettes (schema below the current persist version, including 21) are raised to the current schema before convert / `UpgradeXmlToKpalx`; the version-mismatch dialog is not shown for that path.
  * `KryptonPaletteFile.UpgradeXmlToKpalxFromDirectory` converts every Krypton palette `.xml` in a folder (nested folders included) to `.kpalx` beside the source. Non-palette `.xml` is skipped; one failure does not stop the rest. Designer verb **Upgrade folder .xml to .kpalx...** and `KryptonCustomPaletteBase.UpgradeXmlToKpalxFromDirectory` pick a folder.
  * `KryptonPaletteFile.UpgradeXmlToKpalx` rewrites a legacy `.xml` palette as `.kpalx` in the same folder (source left in place). Prefer `.kpalx`; `.xml` palette files are marked for removal in V120 LTS.
  * `KryptonCustomPaletteBase.UpgradeXmlToKpalx` / `ConvertFile` rewrite the file and import the result into the component. The designer verbs **Upgrade .xml to .kpalx...** and **Convert palette file...** call those APIs. **Upgrade Palette** still only loads into the component (schema XSLT) without rewriting the file.
  * `KryptonPaletteFile.PromptLegacyXmlUpgrade` warns when a legacy `.xml` palette is loaded and offers to rewrite it as `.kpalx` before applying (Yes / No / Cancel). Strings are on `KryptonManager.Strings.MiscellaneousThemeStrings` (`LegacyXmlUpgradeTitle`, `LegacyXmlUpgradeMessage`). Silent imports skip the prompt.
  ...
  * `KryptonPaletteFile.AddToPack` / `RemoveFromPack` add `.kpalx` (or `.kpal` / `.xml`) files to a pack and remove a named theme. Missing packs are created; a single-theme `.kpal` is promoted. The last theme cannot be removed (the pack cannot be empty). `GetPackName` / `SetPackName` read and write the pack header display name.
  * `KryptonPalettePackEditor` is a toolbox component that opens a dialog to add files and remove named themes.
