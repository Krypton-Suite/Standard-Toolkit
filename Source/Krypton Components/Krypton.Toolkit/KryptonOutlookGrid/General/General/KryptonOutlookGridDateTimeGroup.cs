...
/// 
public class KryptonOutlookGridDateTimeGroup : KryptonOutlookGridDefaultGroup
{
    ...
    /// <summary>
    /// The Date Interval of KryptonOutlookGridDateTimeGroup
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="KryptonOutlookGridDateTimeGroup"/> class.
    /// </summary>
    public KryptonOutlookGridDateTimeGroup()
    {
    ...
    public KryptonOutlookGridDateTimeGroup(IKryptonOutlookGridGroup? parentGroup)
        : base(parentGroup)
    {
        AllowHiddenWhenGrouped = true;
        Interval = DateInterval.Smart;
        ...
        set
        {
            switch (Interval)
            {
                case DateInterval.Smart:
                    ...
                    base.Value = KryptonOutlookGridGroupHelpers.GetDayText(_valDateTime);
                    break;
                ...
                case DateInterval.Quarter:
                    if (value != null && value != DBNull.Value)
                    {
                        _valDateTime = DateTime.Parse(value.ToString()!);
                        base.Value =
                            $"{KryptonOutlookGridGroupHelpers.GetQuarterAsString(_valDateTime)} {_valDateTime.Year}";
                    }
                    else
                    {
                        _valDateTime = DateTime.MinValue;
    ...
    /// </summary>
    /// <returns>KryptonOutlookGridDateTimeGroup</returns>
    public override object Clone()
    {
        return new KryptonOutlookGridDateTimeGroup(this);
        /*KryptonOutlookGridDateTimeGroup gr = new(ParentGroup);
        gr.Column = Column;
    ...
    public override int CompareTo(object? obj)
    {
        ...
        else if (obj is KryptonOutlookGridDateTimeGroup)
        {
            val = ((KryptonOutlookGridDateTimeGroup)obj)._valDateTime;
        }
        else
        {
            val = new();
        ...
        {
            case DateInterval.Smart:
                //if (KryptonOutlookGridGroupHelpers.GetDateCode(valDateTime.Date) == KryptonOutlookGridGroupHelpers.GetDateCode(val.Date))
                //{
                //    return 0;
                //}
                //else
                ...
                //}
                return KryptonOutlookGridGroupHelpers.GetDateCodeNumeric(_valDateTime).CompareTo(KryptonOutlookGridGroupHelpers.GetDateCodeNumeric(val)) * orderModifier;
            case DateInterval.Year:
            ...
            case DateInterval.Quarter:
                if (KryptonOutlookGridGroupHelpers.GetQuarter(_valDateTime) == KryptonOutlookGridGroupHelpers.GetQuarter(val) && _valDateTime.Year == val.Year)
                {
                    return 0;
                }
                else
