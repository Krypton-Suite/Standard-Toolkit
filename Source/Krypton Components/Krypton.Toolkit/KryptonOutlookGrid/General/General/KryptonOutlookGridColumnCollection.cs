...
/// </summary>
public class KryptonOutlookGridColumnCollection : List<KryptonOutlookGridColumn>
{
    ...
    /// Constructor
    /// </summary>
    public KryptonOutlookGridColumnCollection()
        : base()
    {
        _maxGroupIndex = -1;
        _maxSortIndex = -1;
    ...
    /// <summary>
    /// Gets the KryptonOutlookGridColumn in the list by its name
    /// </summary>
    /// <param name="columnName">The column name.</param>
    /// <returns>KryptonOutlookGridColumn</returns>
    public KryptonOutlookGridColumn? this[string columnName] => Find( c => c.DataGridViewColumn.Name.Equals(columnName) );
    ...
    /// <summary>
    /// Add an KryptonOutlookGridColumn to the collection.
    /// </summary>
    /// <param name="item">The KryptonOutlookGridColumn to add.</param>
    public new void Add(KryptonOutlookGridColumn item)
    {
        base.Add(item);
    ...
    public List<KryptonOutlookGridColumn>? FindGroupedColumns() => this.Where(c => c.IsGrouped).OrderBy(c => c.GroupIndex).ToList();
    ...
    public List<Tuple<int, SortOrder, IComparer>> GetIndexAndSortGroupedColumns()
    {
        ...
        foreach (KryptonOutlookGridColumn col in tmp)
        {
    ...
    /// <returns>The KryptonOutlookGridColumn.</returns>
    public KryptonOutlookGridColumn? FindFromColumnIndex(int index) => this.FirstOrDefault(c => c.DataGridViewColumn.Index == index);

    /// <summary>
    /// Gets the column from its name
    /// </summary>
    /// <param name="name">The name of the column.</param>
    /// <returns>The associated KryptonOutlookGridColumn.</returns>
    public KryptonOutlookGridColumn? FindFromColumnName(string? name) => this.FirstOrDefault(x => x.Name == name);
    ...
    /// </summary>
    /// <param name="col">The KryptonOutlookGridColumn that will be removed.</param>
    internal void RemoveGroupIndex(KryptonOutlookGridColumn col)
    {
    ...
    /// </summary>
    /// <param name="col">The KryptonOutlookGridColumn that will be removed.</param>
    internal void RemoveSortIndex(KryptonOutlookGridColumn col)
    {
    ...
    internal void ChangeGroupIndex(KryptonOutlookGridColumn outlookGridColumn)
    {
