    ...
    IContainedInputControl
{
    ...
    private void OnTreeViewDrawNode(object? sender, DrawTreeNodeEventArgs e)
    {
        ...
        try
        {
            ...
            Rectangle indentBounds = bounds with { X = bounds.X + nodeIndent - indent, Width = indent };
            bounds.X += nodeIndent;
            ...
            if (hBitmap != IntPtr.Zero)
            {
                try
                {
                    ...
                    // Easier to draw using a graphics instance than a DC!
                    Graphics gfx = Graphics.FromHdc(_screenDC);
                    try
                    {
                        using (var context = new ViewLayoutContext(this, Renderer))
                        {
                        ...
                        using (var context = new RenderContext(this, gfx, e.Bounds, Renderer))
                        {
                            // Do we draw lines between nodes?
                            if (ShowLines
                                && (Redirector.GetMetricBool(PaletteState.Normal, PaletteMetricBool.TreeViewLines) !=
                                    InheritBool.False))
                            {
                                // Find center points
                                ...
                                Color lineColor =
                                    Redirector.GetContentShortTextColor1(PaletteContentStyle.InputControlStandalone,
                                        PaletteState.Normal);
                                using var linePen = new Pen(lineColor);
                                linePen.DashStyle = DashStyle.Dot;
                                linePen.DashOffset = indent % 2;
                                gfx.DrawLine(linePen, hCenter, top, hCenter, bottom);
                                gfx.DrawLine(linePen, hCenter - 1, vCenter - 1, indentBounds.Right, vCenter - 1);
                                hCenter -= indent;
                                ...
                                while (hCenter >= 0)
                                {
                                    ...
                                    begin -= (begin + 1) % 2;
                                    gfx.DrawLine(linePen, hCenter, begin, hCenter, indentBounds.Bottom);
                                    hCenter -= indent;
                                }
                            }
                            ...
                            if (ShowPlusMinus && (e.Node.Nodes.Count > 0))
                            {
                                ...
                                if (drawImage != null)
                                {
                                    float dpiFactor = gfx.DpiX / 96F;
                                    drawImage = CommonHelper.ScaleImageForSizedDisplay(drawImage,
                                        drawImage.Width * dpiFactor,
                                        drawImage.Height * dpiFactor, false)!;
                                    gfx.DrawImage(drawImage, new Rectangle(
                                        indentBounds.X + ((indentBounds.Width - drawImage.Width) / 2) - 1,
                        ...
                        using (var context = new RenderContext(this, gfx, bounds, Renderer))
                        {
                            ...
                            try
                            {
                                if (e.Node.IsSelected)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(e.Node.SelectedImageKey))
                                    {
                                        drawImage = ImageList.Images[e.Node.SelectedImageKey];
                                             ...
                                             (e.Node.SelectedImageIndex < imageCount))
                                    {
                                ...
                                if (drawImage != null)
                                {
                                    ...
                                    drawImage = CommonHelper.ScaleImageForSizedDisplay(drawImage,
                                        drawImage.Width * dpiFactor,
                                        drawImage.Height * dpiFactor, false)!;
                                    gfx.DrawImage(drawImage, _layoutImage.ClientRectangle);
                                }
                            }
                            catch
                            {
                        ...
                        if (_layoutImageCenterState.Visible)
                        {
                            if (drawStateImage != null)
                            {
                                ...
                                gfx.DrawImage(drawStateImage, _layoutImageState.ClientRectangle);
                            }
                        }

                        // Now blit from the bitmap from the screen to the real dc
                        PI.BitBlt(hdc, e.Bounds.X, e.Bounds.Y, e.Bounds.Width, e.Bounds.Height, _screenDC, e.Bounds.X,
                            e.Bounds.Y, PI.SRCCOPY);
                    }
                    finally
                    {
                        gfx.Dispose();
        ...
        {
            // This can happen if the graphics object is disposed or invalid
            // We can safely ignore this exception as it does not affect the control's functionality
        }
        finally
        {
