...
namespace Krypton.Toolkit
{
							   ...
							   IContentValues
	{
		...
		private bool _useWindows11RoundingValue;
		...
		public KryptonForm()
		{
			...
			UseWindows11RoundingValue = false;
		}
		...
		/// <summary> Gets or sets a value indicating whether [use windows11 rounding value].</summary>
		/// <value><c>true</c> if [use windows11 rounding value]; otherwise, <c>false</c>.</value>
		[DefaultValue(false), Description("If you have Windows 11 installed, this will change the corner rounding radius to match the default UI.")]
		public bool UseWindows11RoundingValue
		{
			get => _useWindows11RoundingValue;

			set
			{
				_useWindows11RoundingValue = value;
		...
		{
			if (_useWindows11RoundingValue)
			{
				if (Environment.OSVersion.Version.Major >= 10 && Environment.OSVersion.Version.Build >= 22000)
				{
					CornerRoundingRadius = GlobalStaticValues.WINDOWS_11_PRIMARY_CORNER_ROUNDING_VALUE;
				}
				else
				{
					CornerRoundingRadius = GlobalStaticValues.PRIMARY_CORNER_ROUNDING_VALUE;
