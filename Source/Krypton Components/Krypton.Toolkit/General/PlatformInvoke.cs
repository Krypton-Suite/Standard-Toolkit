...
#pragma warning restore CA1069 // Enums values should not be duplicated
    ...
    internal enum TVIF_
    {
        ...
        DI_SETITEM =
            4096 // The tree-view control will retain the supplied information and will not request it again.This flag is valid only when processing the TVN_GETDISPINFO notification.
    }
    ...
    internal enum TVIS_
    {
        ...
        EXPANDED =
            0x0020, // The item's list of child items is currently expanded; that is, the child items are visible. This value applies only to parent items.
        ...
        SELECTED =
            0x0002, // The item is selected.Its appearance depends on whether it has the focus.The item will be drawn using the system colors for selection.
    ...
    internal enum SIZE_
    {
        ...
        MAXSHOW =
            3, // Message is sent to all pop-up windows when some other window has been restored to its former size.
    ...
    internal struct BS_
    {
        public const uint
            PUSHBUTTON =
                0x0, // Creates a push button that posts a WM_COMMAND message to the owner window when the user selects the button.
            DEFPUSHBUTTON =
                0x1, // +/-Default.Creates a push button with a heavy black border.If the button is in a dialog box, the user can select the button by pressing Enter, even when the button does not have the input focus.This style is useful for enabling the user to quickly select the most likely option.
            CHECKBOX =
                0x2, // Creates a small, empty check box with text.By default, the text is Displayed to the right of the check box.To display the text to the left of the check box, combine this flag with the BS_LEFTTEXT style (or with the equivalent BS_RIGHTBUTTON style).
            AUTOCHECKBOX =
                0x3, // Creates a button that is the same as a check box, except that the check state automatically toggles between checked and cleared each time the user selects the check box.
            RADIOBUTTON =
            ...
            _3STATE =
                0x5, // Creates a button that is the same as a check box, except that the box can be grayed as well as checked or cleared. Use the grayed state to show that the state of the check box is not determined.
            AUTO3STATE =
                0x6, // Creates a button that is the same as a three-state check box, except that the box changes its state when the user selects it.The state cycles through checked, indeterminate, and cleared.AUTOCHECKBOX = 0x3,   // Creates a button that is the same as a check box, except that the check state automatically toggles between checked and cleared each time the user selects the check box.
            GROUPBOX =
                0x7, // Creates a rectangle in which other controls can be grouped. Any text associated with this style is Displayed in the rectangle's upper left corner.
            USERBUTTON =
            ...
            TYPEMASK =
                0xF, // Do not use this style. A composite style bit that results from using the OR operator on BS_* style bits. It can be used to mask out valid BS_* bits from a given bitmask. Note that this is out of date and does not correctly include all valid styles. Thus, you should not use this style.
            LEFTTEXT =
                0x20, // Places text on the left side of the radio button or check box when combined with a radio button or check box style. Same as the BS_RIGHTBUTTON style.
            RIGHTBUTTON =
                0x20, // + Right (i.e. +Right includes both BS_RIGHT and BS_RIGHTBUTTON, but -Right removes only BS_RIGHT, not BS_RIGHTBUTTON). Positions a checkbox square or radio button circle on the right side of the control's available width instead of the left.

            TEXT = 0x0, // Specifies that the button displays text.
            ICON =
                0x40, // Specifies that the button displays an icon. See the Remarks section for its interaction with BS_BITMAP.
            BITMAP =
                0x80, // Specifies that the button displays a bitmap. See the Remarks section for its interaction with BS_ICON
            ...
            PUSHLIKE =
                0x1000, // Makes a checkbox or radio button look and act like a push button.The button looks raised when it isn't pushed or checked, and sunken when it is pushed or checked.
            MULTILINE =
                0x2000, // +/-Wrap.Wraps the text to multiple lines if the text is too long to fit on a single line in the control's available width. This also allows linefeed (`n) to start new lines of text.
            NOTIFY =
                0x4000, // Enables a button to send BN_KILLFOCUS and BN_SETFOCUS notification codes to its parent window. Note that buttons send the BN_CLICKED notification code regardless of whether it has this style.To get BN_DBLCLK notification codes, the button must have the BS_RADIOBUTTON or BS_OWNERDRAW style.
            FLAT =
                0x8000; // Specifies that the button is two-dimensional; it does not use the default shading to create a 3-D effect.
...
No 	                    No 	                    Show text only
    ...
    internal struct WindowCompositionAttribData
    {
        ...
        public IntPtr
            Data; // Will point to an AccentPolicy struct, where Attribute will be WindowCompositionAttribute.AccentPolicy
    ...
    // Toolkit PI uses DllImport only (stub LibraryImportAttribute on older TFMs conflicts with the real one).
    ...
    [DllImport(Libraries.Gdi32, EntryPoint = "RestoreDC", CharSet = CharSet.Auto, SetLastError = true,
        ExactSpelling = true)]
    [DefaultDllImportSearchPaths(DllImportSearchPath.System32)]
    ...
    public class Dwm
    {
        ...
        public enum DWMACCENTSTATE // Affects the rendering of the background of a window.
        {
            ...
            ACCENT_ENABLE_ACRYLICBLURBEHIND =
                4, // Applicable on Win 10 18H1 -> Background is GradientColor, with acrylic blur effect.
        ...
        public static bool IsNonClientRenderingEnabled(IntPtr hWnd)
        {
            var gwaEnabled = 0;
            var result =
                DwmGetWindowAttribute(hWnd, DWMWINDOWATTRIBUTE.NCRenderingEnabled, ref gwaEnabled, sizeof(int));
            return gwaEnabled == 1;
        }
    ...
    private static readonly FieldInfo FIELD_INFO_NATIVE_IMAGE = typeof(Bitmap).GetField(@"nativeImage",
