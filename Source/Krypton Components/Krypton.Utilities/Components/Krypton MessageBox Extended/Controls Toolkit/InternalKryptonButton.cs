...
internal class InternalKryptonButton : VisualSimpleBase, IButtonControl, IContentValues
{
    ...
    // Countdown functionality fields
    private volatile int _countdownSeconds;
    private Timer? _countdownTimer;
    private volatile string? _originalText;
    private CountdownButtonValues? _countdownButtonValues;
    #endregion
    ...
    public CountdownButtonValues CountdownButtonValues
    {
        get
        {
            if (_countdownButtonValues == null)
            {
                _countdownButtonValues = new CountdownButtonValues();
                _countdownButtonValues.IntervalChangedCallback = OnIntervalChanged;
            }
            return _countdownButtonValues;
        }
    }

    private bool ShouldSerializeCountdownButtonValues() => _countdownButtonValues != null && !_countdownButtonValues.IsDefault;
    ...
    public void StartCountdown()
    {
        ...
        if (_countdownTimer == null)
        {
            ...
            {
                Interval = CountdownButtonValues.CountdownInterval
            };
            _countdownTimer.Tick += OnCountdownTimer_Tick;
        }

        _countdownSeconds = CountdownButtonValues.CountdownDuration;
        ...
        if (string.IsNullOrEmpty(_originalText))
        {
            ...
            _originalText = StripCountdownFormatting(Values.Text);
        }

        Enabled = false;
        UpdateCountdownText();

        _countdownTimer.Interval = CountdownButtonValues.CountdownInterval;
    ...
    private string StripCountdownFormatting(string text)
    {
        ...
        string suffix = CountdownButtonValues.CountdownSecondSuffix;
    ...
    private void UpdateCountdownText()
    {
        ...
        string secondsWithSuffix = _countdownSeconds + CountdownButtonValues.CountdownSecondSuffix;
        Values.Text = string.Format(CountdownButtonValues.CountdownTextFormat, _originalText, secondsWithSuffix);
    }
    ...
    private void OnCountdownTimer_Tick(object? sender, EventArgs? e)
    {
        try
        {
            ...
            else
            {
                ...
                if (CountdownButtonValues.EnableButtonAtZero)
                {
                    UpdateCountdownSafely(() =>
                    {
                        Enabled = true;
                ...
                else
                {
                    ...
                    {
                        Enabled = true;
                        Values.Text = _originalText ?? string.Empty;
                        OnCountdownFinished(EventArgs.Empty);
                    });
                }
            }
