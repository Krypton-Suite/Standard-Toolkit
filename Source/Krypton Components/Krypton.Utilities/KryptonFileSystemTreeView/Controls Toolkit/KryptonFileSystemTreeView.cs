...
[DefaultProperty(nameof(FileSystemValues.RootPath))]
...
public class KryptonFileSystemTreeView : KryptonTreeView
{
    ...
    /// </value>
    public FileSystemTreeViewValues FileSystemValues => _fileSystemValues;
    ...
    public void Reload()
    {
        ...
        if (string.IsNullOrEmpty(FileSystemValues.RootPath) || !Directory.Exists(FileSystemValues.RootPath))
        {
            return;
        }
        ...
        try
        {
            TreeNode rootNode = CreateDirectoryNode(FileSystemValues.RootPath);
            Nodes.Add(rootNode);
            rootNode.Expand();
        }
        catch (Exception ex)
        {
            OnFileSystemError(new FileSystemErrorEventArgs(FileSystemValues.RootPath, ex));
        }
    }
    ...
    private int GetIconIndex(string path, bool isDirectory)
    {
        ...
        try
        {
            if (isDirectory)
            {
                ...
                {
                    icon = FileSystemIconHelper.GetFileSystemIcon(path, FileSystemValues.UseLargeIcons);
                }

                // Fallback to generic folder icon
                if (icon == null)
                {
                    icon = FileSystemIconHelper.GetFolderIcon(FileSystemValues.UseLargeIcons);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        icon = FileSystemIconHelper.GetFileIcon(extension, FileSystemValues.UseLargeIcons);
                    }
                }
        ...
        try
        {
            ...
            foreach (string dir in directories)
            {
                try
                {
                    ...
                    if ((isHidden && !FileSystemValues.ShowHiddenFiles) || (isSystem && !FileSystemValues.ShowSystemFiles))
                    {
                        continue;
                    }
            ...
            // Get files
            if (FileSystemValues.ShowFiles)
            {
                string[] files = Directory.GetFiles(directoryPath, FileSystemValues.FileFilter);
