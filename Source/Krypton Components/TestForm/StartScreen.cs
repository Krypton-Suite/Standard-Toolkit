...
public partial class StartScreen : KryptonForm
{
    ...
    private void AddButtons()
    {
        ...
        CreateButton<CustomThemeGeneratorDemo>("4234 Custom Theme Generator", "Issue #4234: generate a custom theme from a few colours (hex, RGB, or a screen dropper). Switch the picker flyout between Classic (PowerToys) and Krypton chrome. Apply, register, export .kpalx/XML, and open the Utilities builder.");
        ...
        CreateButton<PaletteBinaryDemo>("2117 Palette .kpalx save/load", "Issue #2117: export/import custom palettes as .kpalx (KryptonPalette XML). Loading a legacy .xml file warns and offers to upgrade to .kpalx before applying (localisable via MiscellaneousThemeStrings). Populate from a Krypton.Themes extra palette, re-import, and apply via KryptonManager. Upgrade .xml to .kpalx uses KryptonCustomPaletteBase.UpgradeXmlToKpalx. Upgrade folder .xml to .kpalx uses UpgradeXmlToKpalxFromDirectory. Convert XML to .kpalx uses ConvertFile. Export .kpal pack stores several named themes. Pack folder to .kpal stores a directory tree. Edit .kpal pack opens KryptonPalettePackEditor (Utilities) to add .kpalx files and remove named themes. KryptonPaletteFileListBox / ComboBox / TreeView (Utilities) list those files from the last export folder. File dialogs default to *.kpalx; optional native *.kpal is listed; *.xml remains available until V120 LTS. For a dedicated editor use Demos Krypton Palette Author (Krypton Explorer).");
        CreateButton<PalettePackEditorDemo>("2117 Palette pack editor", "Issue #2117: KryptonPalettePackEditor in Krypton.Toolkit.Utilities adds .kpalx files to a .kpal pack and removes named themes (AddToPack / RemoveFromPack). Create a sample pack, Add Pack-Violet.kpalx, Remove it, and confirm the last theme cannot be removed.");
