...
public sealed class Bug1870CustomPaletteBaseDemo : KryptonForm
{
    ...
    {
        foreach (KeyValuePair<string, PaletteMode> pair in PaletteModeStrings.SupportedThemesMap)
        {
            if (pair.Value is PaletteMode.Custom or PaletteMode.Global)
            {
                continue;
            }

            _cboBaseMode.Items.Add(new ModeItem(pair.Value, pair.Key));
        }
