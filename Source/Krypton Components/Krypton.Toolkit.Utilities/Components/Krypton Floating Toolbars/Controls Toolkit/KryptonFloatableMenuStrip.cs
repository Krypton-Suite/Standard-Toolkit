...
[ToolboxBitmap(typeof(KryptonFloatableMenuStrip), "ToolboxBitmaps.FloatableToolStrip.bmp")]
public partial class KryptonFloatableMenuStrip : KryptonMenuStrip
{
    ...
    private bool _showFloatingWindowControlBox;

    private VisualMenuStripContainerForm? _menuStripContainerForm;

    private Control? _originalParent;

    private List<KryptonMenuStripPanelExtended>? _menuStripPanelExtendedList = [];
    ...
    /// <summary>
    /// Occurs when the floating state of the menu strip changes.
    /// </summary>
    ...
    /// </value>
    [Editor(typeof(KryptonMenuStripPanelCollectionEditor), typeof(UITypeEditor)), DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
    public List<KryptonMenuStripPanelExtended>? MenuStripPanelExtendedList { get => _menuStripPanelExtendedList; set => _menuStripPanelExtendedList = value; }
    ...
    public new bool Visible
    {
        ...
        set
        {
            if (_isFloating)
            {
                _menuStripContainerForm?.Visible = value;
            }
            else
            {
                base.Visible = value;
    ...
    /// <returns>True if the menu strip was successfully floated; false if it was already floating or cannot be floated.</returns>
    public bool Float(Point? location = null)
    {
        ...
        if (_originalParent == null && Parent != null && !(Parent is VisualFloatingContainerForm))
        {
            _originalParent = Parent;
        }

        // Create container form if needed
        if (_menuStripContainerForm == null)
        {
            _menuStripContainerForm = new VisualMenuStripContainerForm();
            _menuStripContainerForm.Text = FloatingWindowText;
            _menuStripContainerForm.NCLBUTTONDBLCLK += MenuStripContainerForm_NCLBUTTONDBLCLK;
            _menuStripContainerForm.LocationChanged += MenuStripContainerForm_LocationChanged;
            _menuStripContainerForm.FormClosing += MenuStripContainerForm_FormClosing;
            _menuStripContainerForm.MouseUp += MenuStripContainerForm_MouseUp;
        }
        ...
        _menuStripContainerForm.KryptonFloatableMenuStrip = this;
        
        // Apply window style
        ApplyWindowStyle(_menuStripContainerForm);
        ...
        {
            _menuStripContainerForm.Show(_originalParent.Parent);
        }
        else if (_originalParent != null)
        {
            _menuStripContainerForm.Show(_originalParent);
        }
        else
        {
            _menuStripContainerForm.Show();
        }
        ...
        if (_enableAnimation && _animationDuration > 0)
        {
            ...
            _menuStripContainerForm.Location = startLocation;
            _menuStripContainerForm.Opacity = 0;
            
            FloatingAnimationHelper.AnimateForm(_menuStripContainerForm, targetLocation, 1.0, _animationDuration);
        }
        else
        {
            _menuStripContainerForm.Location = targetLocation;
        }
    ...
    /// <returns>True if the menu strip was successfully docked; false if it was not floating or cannot be docked.</returns>
    public new bool Dock(KryptonMenuStripPanelExtended? targetPanel = null)
    {
        if (!_isFloating)
        {
        ...
        if (targetPanel != null)
        {
            ...
            if (_menuStripPanelExtendedList == null || !_menuStripPanelExtendedList.Contains(targetPanel))
            {
        ...
        _menuStripContainerForm?.Controls.Remove(this);
        _menuStripContainerForm?.Hide();
    ...
    public FloatingToolbarState? SaveState()
    {
        ...
        if (_isFloating && _menuStripContainerForm != null)
        {
            state.Location = _menuStripContainerForm.Location;
            state.Size = _menuStripContainerForm.Size;
        }
        else if (Parent != null)
        {
            ...
            if (Parent is KryptonMenuStripPanelExtended panel)
            {
    ...
    public bool LoadState(FloatingToolbarState? state)
    {
        ...
        if (state.IsFloating)
        {
            // Float the menu strip at the saved location
            if (Float(state.Location))
            {
                if (_menuStripContainerForm != null && !state.Size.IsEmpty)
                {
                    _menuStripContainerForm.Size = state.Size;
                }
                return true;
            }
        }
        else
        {
            ...
            {
                var targetPanel = _menuStripPanelExtendedList?.FirstOrDefault(p => p.Name == state.DockedPanelName);
                if (targetPanel != null)
                {
    ...
    private void ApplyWindowStyle(VisualMenuStripContainerForm form)
    {
    ...
    {
        if (_menuStripContainerForm == null || _windowTheme == null)
        {
            return;
        }
        ...
        _menuStripContainerForm.Invalidate();
    }
    #endregion
    ...
    protected override void OnParentChanged(EventArgs e)
    {
        ...
        if (Parent != null)
        {
            if (!(Parent is VisualFloatingContainerForm))
            {
                _originalParent = Parent;
    ...
    protected override void OnMouseUp(MouseEventArgs mea)
    {
        ...
        if (_originalParent != null)
        {
            ...
            if (_aboutToFloat && !_originalParent.ClientRectangle.Contains(p1))
            {
                ...
                _menuStripContainerForm.Location = p0;

                if (Parent != null)
                {
                    _menuStripContainerForm.Show(Parent.Parent);
                }
    ...
    private void MenuStripContainerForm_FormClosing(object? sender, FormClosingEventArgs e)
    {
    ...
    private void MenuStripContainerForm_LocationChanged(object? sender, EventArgs e)
    {
        Point point;
        ...
        if (_menuStripPanelExtendedList != null)
        {
            foreach (KryptonMenuStripPanelExtended item in _menuStripPanelExtendedList)
            {
        ...
        if (!foundDockZone && _menuStripContainerForm != null)
        {
    ...
    private void MenuStripContainerForm_MouseUp(object? sender, MouseEventArgs e)
    {
            ...
            foreach (var group in FloatingToolbarGroupManager.Groups)
            {
                if (group is { IsTabbed: true, TabbedContainerForm: not null })
                {
                    ...
                    if (containerRect.Contains(point))
                    {
                        ...
                        // Move menu strip to group
                        FloatingToolbarGroupManager.MoveMenuStripToGroup(this, group);
                        
                        // If group is tabbed, ensure it's visible
                        if (group.IsTabbed && containerForm.Visible)
                        {
                            // Menu strip will be added to tabbed container automatically
                            return;
                        }
                    }
                }
    ...
    private void MenuStripContainerForm_NCLBUTTONDBLCLK(object? sender, EventArgs e)
    {
        Dock(); // Dock to original parent
    }
    ...
    /// </summary>
    /// <param name="isFloating">True if the menu strip is now floating; false if docked.</param>
