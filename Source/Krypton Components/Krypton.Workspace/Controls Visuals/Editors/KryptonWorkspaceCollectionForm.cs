...
internal partial class KryptonWorkspaceCollectionForm : VisualDesignerCollectionForm
{
    ...
    private void OnVisibleChanged(object? sender, EventArgs e)
    {
            ...
            if (node != null)
            {
                ...
                if (PreviousNode(node) is MenuTreeNode previousNode)
                {
                    ...
                    if (isNodePage)
                    {
                        ...
                        var parentNode =node.Parent as MenuTreeNode ?? ThrowHelper.ThrowNullReferenceException<MenuTreeNode>(SharedStaticFunctions.VariableCannotBeNull(nameof(node.Parent)));
        ...
        private void buttonAddSequence_Click(object? sender, EventArgs e)
        {
            ...
            var sequence =CreateInstance(typeof(KryptonWorkspaceSequence)) as KryptonWorkspaceSequence ?? ThrowHelper.ThrowNullReferenceException<KryptonWorkspaceSequence>(SharedStaticFunctions.VariableCannotBeNull("sequence"));
            ...
            else if (selectedNode.CellItem != null)
            {
                ...
                {
                    // Selected node is a cell, so insert after this cell
                    var selectedParentNode =selectedNode.Parent as MenuTreeNode ?? ThrowHelper.ThrowNullReferenceException<MenuTreeNode>(SharedStaticFunctions.VariableCannotBeNull(nameof(selectedNode.Parent)));
                    var selectedIndex = selectedParentNode.Nodes.IndexOf(selectedNode);
            ...
            out IWorkspaceItem? before)
        {
            ...
            var beforeSequence =after.WorkspaceParent as KryptonWorkspaceSequence ?? ThrowHelper.ThrowNullReferenceException<KryptonWorkspaceSequence>(SharedStaticFunctions.VariableCannotBeNull(nameof(after.WorkspaceParent)));
