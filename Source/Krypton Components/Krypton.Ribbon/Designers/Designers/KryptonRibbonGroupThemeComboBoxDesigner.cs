...
internal class KryptonRibbonGroupThemeComboBoxDesigner : ComponentDesigner, IKryptonDesignObject
{
    ...
    private void OnEnabled(object? sender, EventArgs e)
    {
        if (_ribbonThemeComboBox.Ribbon is not null)
        {
            PropertyDescriptor? propertyEnabled = TypeDescriptor.GetProperties(_ribbonThemeComboBox)[nameof(Enabled)] ?? throw new NullReferenceException(GlobalStaticFunctions.VariableCannotBeNull("propertyEnabled"));
    ...
    private void OnVisible(object? sender, EventArgs e)
    {
        if (_ribbonThemeComboBox.Ribbon is not null)
        {
            PropertyDescriptor? propertyVisible = TypeDescriptor.GetProperties(_ribbonThemeComboBox)[nameof(Visible)] ?? throw new NullReferenceException(GlobalStaticFunctions.VariableCannotBeNull("propertyVisible"));
