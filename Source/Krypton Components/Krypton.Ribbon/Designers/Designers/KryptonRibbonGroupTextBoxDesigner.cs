...
internal class KryptonRibbonGroupTextBoxDesigner : ComponentDesigner, IKryptonDesignObject
{
    ...
    private void OnEnabled(object? sender, EventArgs e)
    {
        if (_ribbonTextBox.Ribbon is not null)
        {
            PropertyDescriptor? propertyEnabled = TypeDescriptor.GetProperties(_ribbonTextBox)[nameof(Enabled)] ?? throw new NullReferenceException(GlobalStaticFunctions.VariableCannotBeNull("propertyEnabled"));
    ...
    private void OnVisible(object? sender, EventArgs e)
    {
        if (_ribbonTextBox.Ribbon is not null)
        {
            PropertyDescriptor? propertyVisible = TypeDescriptor.GetProperties(_ribbonTextBox)[nameof(Visible)] ?? throw new NullReferenceException(GlobalStaticFunctions.VariableCannotBeNull("propertyVisible"));
