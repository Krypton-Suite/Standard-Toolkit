...
internal class ButtonSpecRibbonController : ButtonController
{
    ...
    public override void KeyDown(Control c, KeyEventArgs e)
    {
        ...
        if (ribbon is null)
        {
            throw new NullReferenceException(SharedStaticFunctions.VariableCannotBeNull(nameof(ribbon)));
        }
        ...
        {
            throw new NullReferenceException(SharedStaticFunctions.PropertyCannotBeNull(nameof(ribbon.TabsArea.ButtonSpecManager)));
        }

        // Get the button spec associated with this controller
        ViewDrawButton? viewButton = Target as ViewDrawButton ?? throw new NullReferenceException(SharedStaticFunctions.VariableCannotBeNull(nameof(Target)));
