<#
.SYNOPSIS
    Opens Feature4339ComboBoxSimpleStyleDemo and captures a PNG for #4424 (V105-LTS).

.DESCRIPTION
    Loads TestForm assemblies in-process (STA), shows the combo demo (bottom DropDownList
    exercises the edit-strip GDI+ DrawString path), captures the window, and writes
    Documents/PR/4424-combobox-drawstring-v105.png.
...
if (-not $OutputPath) {
    $OutputPath = Join-Path $repoRoot 'Documents\PR\4424-combobox-drawstring-v105.png'
}
...
# Bottom-right DropDownList exercises the edit-strip GDI+ DrawString path (#4424).
